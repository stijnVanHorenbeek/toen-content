...
Optional `.toen/releases/requests/<build-uuid>.json` files correlate one Cloudflare build with exact application and content SHAs. Mutable `.toen/releases/activation-lock.json` serializes trigger/deploy ownership without adding KV, Queues, or Durable Objects. Validator accepts these reader-first records before publication writer is enabled. Records are strict, at most 4,096 bytes, path/UUID bound where applicable, symlink-contained, and SHA-256 integrity checked. Release records never replace canonical event Markdown.
